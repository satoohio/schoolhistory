...
export default function Home() {
  ...
  return (
    <div style={{ paddingTop: 64 }}>
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', bottom: -1, left: 0, right: 0 }}>
          <svg viewBox="0 0 1440 80" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
            <path d="M0 80L1440 80L1440 35C1100 -5 640 75 0 18L0 80Z" fill="#f8fafc" />
          </svg>
        </div>

        <div className="wrap" style={{ position: 'relative', zIndex: 1, padding: '80px 24px 120px' }}>
          <div style={{ maxWidth: 600 }}>
            ...
            <h1 className="fade-up" style={{
              ...
              Знания.<br />
              <span style={{ color: '#fbbf24' }}>Творчество.</span><br />
              Будущее.
            </h1>
            ...
            <div className="fade-up" style={{ display: 'flex', gap: 12, flexWrap: 'wrap', animationDelay: '0.15s' }}>
              <Link to="/gallery" className="btn-primary" style={{ background: '#fbbf24', color: '#1e293b' }}>
      ...
      {/* ── Stats ────────────────────────────────────────── */}
      <section className="wrap" style={{ marginTop: -40, position: 'relative', zIndex: 10, paddingBottom: 0 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }} className="stats-grid">
          {stats.map(({ icon: Icon, value, label, color }) => (
            <div key={label} className="stat-card">
              <div style={{ width: 40, height: 40, background: color + '18', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
      ...
      {featured.length > 0 && (
        <section className="section wrap">
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
            <div>
              <div style={{ fontSize: '0.8rem', fontWeight: 600, color: '#3b82f6', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>Школьная жизнь</div>
              <h2 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#0f172a', letterSpacing: '-0.02em' }}>Яркие моменты</h2>
            </div>
            <Link to="/gallery" style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#1d4ed8', fontSize: '0.9rem', fontWeight: 500 }}>
              Вся галерея <ArrowRight size={15} />
            </Link>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }} className="featured-grid">
              ...
              >
                ...
                <div className="photo-overlay">
                  <div style={{ position: 'absolute', bottom: 12, left: 12, right: 12 }}>
                    <p style={{ color: '#fff', fontWeight: 600, fontSize: '0.875rem', marginBottom: 2 }}>{photo.title}</p>
                    {photo.category_name && <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.75rem' }}>{photo.category_name}</p>}
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* ── CTA ──────────────────────────────────────────── */}
      <section style={{ background: 'linear-gradient(135deg, #1d4ed8, #7c3aed)', padding: '72px 0' }}>
        <div className="wrap" style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '1.9rem', fontWeight: 800, color: '#fff', marginBottom: 12, letterSpacing: '-0.02em' }}>Присоединяйтесь к нашему сообществу</h2>
          <p style={{ color: 'rgba(255,255,255,0.72)', fontSize: '1rem', marginBottom: 32, maxWidth: 440, margin: '0 auto 32px' }}>
            Зарегистрируйтесь, чтобы получить доступ к полной галерее и новостям школы
          </p>
          <Link to="/register" style={{
            display: 'inline-flex', alignItems: 'center', gap: 8,
            padding: '13px 28px', background: '#fff', color: '#1d4ed8',
            fontWeight: 700, borderRadius: 12, fontSize: '0.95rem',
